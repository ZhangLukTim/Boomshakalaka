 ...
 */
public class GameviewUI extends Canvas {
    ...
    public GameviewUI(GameToolBar gameToolBar) {
    ...
    private void setupGameBoard() {
        ...
        this.mouseSteering = new MouseSteering(this, this.gameBoard.getSpaceship());
    }

    private void setupImageCache() {
        this.imageCache = new HashMap<>();
        for (Debris debris : this.gameBoard.getDebrisList()) {
            String imageLocation = debris.getIconLocation();
     ...
     *
     * @param debris to be drawn
     */
    private void paintDebris(Debris debris) {
        Point2D debrisPosition = debris.getPosition();

        getGraphicsContext2D().drawImage(this.imageCache.get(debris.getIconLocation()), debrisPosition.getX(),
                debrisPosition.getY(), debris.getSize().getWidth(), debris.getSize().getHeight());
    }
}
